...
import { CommitmentRow, YearlyOutstanding, PageView } from './types';
import { LayoutDashboard, Table2, BarChart3, Wallet, Menu, CalendarClock } from 'lucide-react';
...
function App() {
  ...
  const [yearlyData, setYearlyData] = useState<YearlyOutstanding[]>(
  ...
  const NavItem = ({ id, label, icon: Icon }: { id: PageView, label: string, icon: any }) => (
    <button
      onClick={() => setView(id)}
